...
import { styles, topBarIconStyle } from "./BartenderManagementPageStyles.js";
...
export default class BartenderManagementPage extends Component {
  ...
  getItems = async () => {
    http
      ...
      .then((response) => {
        ...
        var enabled = fetchedList.filter(
          (item) => item.productState === "ON_STORE"
        );
        var disabled = fetchedList.filter(
          (item) => item.productState !== "ON_STORE"
        );
        this.setState({
  ...
  sendRequestEnableItem = (id) => {
    http
      ...
      .then((response) => {
        var actualEnabledItems = this.state.enabledItems;
        actualEnabledItems.push(response.data);
        var actualDisabledItems = this.state.disabledItems;
