...
export default class BartenderSettingsPage extends Component {
  ...
  sendRequestEnableItem = (id) => {
    http.post('/product/enable/' + id).then((response) => {
      actualEnabledItems = this.state.enabledItems;
      items.push(response.data);
      actualDisabledItems = this.state.disabledItems;
  ...
  renderItem = ({item}) => {
    var actionIcon = item.onStore ? <AntDesign name="close" size={36} color="black" onPress={ () => disableFunc(item.id) } />
