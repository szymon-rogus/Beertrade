...
export default class BartenderOrderPage extends Component {
  ...
  renderItem = ({item}) => {
    var shadowLayerValue = this.state.lastOrderPresent && item == this.state.lastOrder ? <View style={ styles.lastOrderLayer }>
    <Ionicons name="md-refresh" size={40} color="black" onPress={ this.restoreItem } />
    </View> : null;
    return (
      <OrderItem item={ item } executeFunc = { this.sendRequestExecuteItem } cancelFunc = { this.sendRequestCancelItem } shadowLayer = { shadowLayerValue }/>
    );
  };
      ...
      <View style={ globalStyles.mainContainer }>
        <View style={ styles.topBar }>
          ...
          <View style={ styles.navIconsBox }>
            <Ionicons name="md-settings" size={ 36 } color="white" style={{margin: 4}} onPress={() => this.props.navigation.navigate('bartenderSettings')} />
