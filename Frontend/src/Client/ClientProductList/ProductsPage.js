...
const Item = ({item, onPress, navigation, buttonEnabled, isTableSet, price}) => (
    <View style={styles.item}>
      ...
      >
        ...
        <Text style={styles.attributes}>{asMoney(price)} {CURRENCY}</Text>
        <TouchableOpacity
    ...
    let filteredData = this.state.products.filter(function (item) {
      return (
          searchText.length === 0 ? false :
              item.name.substring(0, searchText.length).toLowerCase() === searchText.toLowerCase()
      );
    });
  ...
  sortByChosenAttr = (list) => {
    list.sort((a, b) => {
      if (this.state.sortBy === 'Name') {
        return this.state.sortAsc ? a.name.toLowerCase() > b.name.toLowerCase()
      ...
      if (this.state.sortBy === 'Alcohol') {
        return this.state.sortAsc ? a.alcoholPercentage > b.alcoholPercentage
