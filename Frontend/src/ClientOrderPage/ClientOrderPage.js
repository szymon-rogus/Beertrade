  ...
  <View style={listStyles.item}>
    ...
    >
      <View style={listStyles.attributeView}>
        ...
        <Text style={{ fontSize: 14, color: "black", marginLeft: 40 }}>
          {item.beerName}
        </Text>
      </View>
      <View style={listStyles.attributeView}>
        <Text style={listStyles.itemLabelText}>Amount:</Text>
        <Text style={{ fontSize: 14, color: "black", marginLeft: 27 }}>
          {item.amount}
        </Text>
      </View>
      <View style={listStyles.attributeView}>
        <Text style={listStyles.itemLabelText}>Table:</Text>
        <Text style={{ fontSize: 14, color: "black", marginLeft: 42 }}>
          {item.tableNumber}
        </Text>
      </View>
      <View style={listStyles.attributeView}>
        <Text style={listStyles.itemLabelText}>User:</Text>
        <Text style={{ fontSize: 14, color: "black", marginLeft: 48 }}>
          {item.userLogin}
        </Text>
      </View>
    </View>
  ...
  sendRequestExecuteItem = (id) => {
    http
      ...
      .then(() => {
        ...
        newLastOrder = actualItems.filter((item) => item.id == id)[0];
    ...
    return (
      <View style={globalStyles.mainContainer}>
        <View style={styles.topBar}>
          ...
          <View style={styles.navIconsBox}>
            <Ionicons
              ...
              style={{ margin: 4 }}
              onPress={() =>
                this.props.navigation.navigate("bartenderSettings")
              }
            />
        ...
        <View style={styles.bartenderStatisticsBox}>
          <Progress.Bar
            progress={this.state.done / this.state.maxOrders}
            width={320}
            height={10}
            animated={true}
          />
          <View style={styles.orderStatsInsideBox}>
            ...
            <Text style={{ fontSize: 16, color: "black", paddingLeft: 50 }}>
              {this.state.waiting}
            </Text>
          </View>
          <View style={styles.orderStatsInsideBox}>
            <Text style={styles.statsText}>Done</Text>
            <Text style={{ fontSize: 16, color: "black", paddingLeft: 72 }}>
              {this.state.done}
            </Text>
          </View>
          <View style={styles.orderStatsInsideBox}>
            <Text style={styles.statsText}>Cancelled</Text>
            <Text style={{ fontSize: 16, color: "black", paddingLeft: 40 }}>
